...
import { MAX_LEVEL, makeRound } from './wordPictureLogic'

export default function WordPictureMatch({ profileId }) {
  const { level, stars, setAnswered, setSize, recordAnswer } = useGameProgress(profileId, 'tierB-word-picture', {
    maxLevel: MAX_LEVEL,
    targetTimeMs: 12000,
  })
  ...
  function choose(value) {
    ...
    if (correct) {
      playSuccess()
      speak(`Correct! This is a ${round.answer}.`, 'en-US')
      setFeedback('🎉 Correct!')
    } else {
      playGentle()
  ...
  return (
    <div className="game-area english-game">
      ...
      <div className="emoji-prompt">{round.emoji}</div>
